...
**54:** Found that `/clients` was never broken. Every "MyCase's contacts endpoint is timing out" refusal traced to OUR scan-cap probe: it hardcoded `page_size=1`, and **MyCase hangs on `/clients` for page_size 1 and 2 (~46s timeout, reproducible) while 5, 10, 25, 100 and 1000 all return in under a second reporting the same `item_count` of 8,499.** The probe was the only thing failing, and it made the whole endpoint look permanently dead — so duplicate-contact detection, contacts-by-date and every client question were written off for a day as an unavoidable MyCase limitation, and the agent was even taught to say so. All six scan-cap probes now use `_PROBE_PAGE_SIZE = 5`. **Audited all 21 firm-wide list endpoints, 3 runs per page size: `/clients` is the ONLY one affected** — deterministically so (size 1 and 2 fail 3/3, size 5 succeeds 3/3 in 0.8s). A first audit pass appeared to implicate several others, but that was a too-tight probe timeout clipping slow-but-working responses; don't trust a single failed sample. That pass did surface something real though: **this account is slow and wildly variable — the same request with the same parameters ranged 0.9s to 22.6s** (get_tasks: 22.2s/1.2s/21.9s). The per-attempt timeout was raised 25s -> 35s so a legitimate 22.6s response is never aborted and retried into an even shorter window; the 45s total budget is unchanged, so the proxy ceiling still holds. Unblocked immediately: 8,499 clients queryable, **3,183 with no email address**, 980 duplicate-phone groups. Added `field_filters` to `aggregate_clients` with the same ""/"*"/substring conventions so "clients without an email address" is one direct call, and removed the now-false limitation text from the prompt and tool descriptions. Lesson worth keeping: a probe that fails is not proof the endpoint fails — vary the probe before declaring an integration broken. 131 tests pass.
